...
function ProductView({ product }: { product: ProductWithImages }) {
  ...
  useEffect(() => {
    trackMetaEvent("ViewContent", {
      ...
      currency: "XOF",
      value: product.price_fcfa,
    });
  }, [product.id, product.name, product.price_fcfa]);
  ...
  useEffect(() => {
    ...
    const nextAvailable =
      product.sizes.find((s) => !product.sold_out_sizes?.includes(s)) ?? null;
    setSize(nextAvailable);
    if (nextAvailable) {
      setSizeError(
        `Taille ${size} épuisée — taille ${nextAvailable} sélectionnée.`,
      );
    }
  ...
  const relatedProducts = useMemo(() => {
    return allProducts
      ...
      .sort((a, b) => {
        ...
        return (
          Math.abs(a.price_fcfa - product.price_fcfa) -
          Math.abs(b.price_fcfa - product.price_fcfa)
        );
      })
      .slice(0, 4);
  ...
  const handleAddToCart = () => {
    ...
    if (size && product.sold_out_sizes?.includes(size)) {
      setSizeError(
        `La taille ${size} est épuisée. Veuillez en choisir une autre.`,
      );
      return;
    }
    setSizeError(null);
    addToCart({
      ...
      soldOutSizes: product.sold_out_sizes ?? [],
      fit,
      lapel,
      lining,
    ...
    <div className="bg-ink pt-32 sm:pt-40 pb-20 sm:pb-32 overflow-x-hidden">
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-12 max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-10 sm:gap-12 lg:gap-20">
        ...
        <div className="lg:sticky lg:top-32 self-start">
          ...
          <h1 className="display text-3xl sm:text-4xl md:text-5xl mb-6 break-words">
            {product.name}
          </h1>
          {product.story && (
          ...
          <div className="border-y border-hairline py-6 mb-10">
            <div className="text-bone text-lg font-light">
              ...
              <Section label="Size">
                <TooltipProvider delayDuration={150}>
                  <div className="flex flex-wrap gap-2">
                    {product.sizes.map((s) => {
                      const soldOut =
                        product.sold_out_sizes?.includes(s) ?? false;
                      const chip = (
                        <Chip
                          key={s}
                          active={size === s}
                        ...
                        <Tooltip key={s}>
                          ...
                          >
                            <p className="text-xs leading-relaxed">
                              La taille <span className="font-medium">{s}</span>{" "}
                ...
                <div className="mt-4">
                  <Dialog>
                    ...
                    <DialogContent className="bg-ink border-hairline text-bone max-w-xl">
                      <DialogHeader>
                        <DialogTitle className="font-serif text-2xl text-bone">
                          Size Finder
                        </DialogTitle>
                      </DialogHeader>
                      <div className="pt-2">
                        <SizeFinder
                          availableSizes={product.sizes.filter(
                            (s) => !product.sold_out_sizes?.includes(s),
                          )}
                          onPick={(s) => {
                            setSize(s);
                            setSizeError(null);
                ...
                {actuallySoldOut && (
                  <>
                    <p className="text-xs text-bone/60 mt-3 tracking-wider font-light">
                      Toutes les tailles sont actuellement épuisées. Soyez
                      prévenu dès que la pièce revient.
                    </p>
                    <RestockAlertForm
                      productSlug={product.slug}
                      productName={product.name}
                      size={size}
                      expectedRestockDate={
                        new Date(Date.now() + 6 * 7 * 24 * 60 * 60 * 1000)
                      }
                    />
                  </>
                )}
                {sizeError && (
                  <p
                    role="alert"
                    className="text-xs text-red-400/90 mt-3 tracking-wider"
                  >
                    {sizeError}
                  </p>
                )}
          ...
          {product.monogram && (
            <Section label="Monogram (optional)">
              <input
                value={monogram}
                onChange={(e) =>
                  setMonogram(e.target.value.slice(0, 4).toUpperCase())
                }
          ...
          <div className="mt-10 flex flex-col gap-3">
            {sizeError && (
              <p
                role="alert"
                className="text-xs text-red-400/90 tracking-wider"
              >
                {sizeError}
              </p>
            )}
        ...
        <div className="px-4 sm:px-6 md:px-8 lg:px-12 max-w-[1600px] mx-auto mt-20 sm:mt-32 grid grid-cols-1 md:grid-cols-2 gap-12 sm:gap-16">
          {(product.fabric_composition ||
            ...
            <div>
              ...
              <h2 className="display text-2xl sm:text-3xl md:text-4xl mb-6 sm:mb-8">
                Fabric
              </h2>
              <dl className="space-y-5">
                {product.fabric_composition && (
                  <Row k="Composition" v={product.fabric_composition} />
                )}
                {product.fabric_weight && (
                  <Row k="Weight" v={product.fabric_weight} />
                )}
                {product.fabric_mill && (
                  <Row k="Mill" v={product.fabric_mill} />
                )}
              </dl>
              {product.fabric_notes && (
                <p className="text-bone/60 font-light leading-relaxed mt-8">
                  {product.fabric_notes}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <ul className="space-y-4">
                {product.details.map((d) => (
                  <li
                    key={d}
                    className="flex gap-4 text-bone/70 font-light leading-relaxed border-b border-hairline pb-4"
                  >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {relatedProducts.map((p) => (
              ...
              >
                ...
                <div className="eyebrow text-bone/50 text-[10px] mb-2">
