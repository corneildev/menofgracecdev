...
function RestockAlertsAdmin() {
  ...
  const loadAlerts = async () => {
    ...
    const { data, error } = await supabase
      .from("restock_alerts")
      .select("id,email,whatsapp,product_id,product_slug,product_name,size,notified,created_at")
  ...
  const filtered = useMemo(() => {
    ...
    return alerts.filter((a) => (filter === "notified" ? a.notified : !a.notified));
  }, [alerts, filter]);
  ...
  const setNotified = async (id: string, notified: boolean) => {
    ...
    setAlerts((prev) => prev.map((a) => (a.id === id ? { ...a, notified } : a)));
    toast.success(notified ? "Marquée comme notifiée" : "Marquée comme en attente");
  };

  const markAllForProduct = async (productId: string | null, productSlug: string | null) => {
    const targets = alerts.filter(
      (a) => !a.notified && ((productId && a.product_id === productId) || (productSlug && a.product_slug === productSlug)),
    );
    ...
    const { error } = await supabase
      ...
      .in("id", ids);
    if (error) {
      toast.error("Action groupée impossible");
      return;
    }
    ...
    <div className="pt-40 pb-32 px-6 md:px-12 bg-ink min-h-screen">
      <div className="max-w-[1400px] mx-auto">
        <div className="border-b border-hairline pb-8 mb-12 flex flex-wrap items-end justify-between gap-6">
          <div>
            <Link to="/admin" className="eyebrow text-bone/50 text-[10px] hover:text-bone">← Console</Link>
            ...
            <p className="text-bone/60 font-light mt-3 max-w-xl">
              Suivez les clients en attente d'une remise en stock et marquez les demandes comme notifiées une fois contactés.
            </p>
          </div>
          <div className="flex items-center gap-2" role="group" aria-label="Filtre">
            {(
              [
        ...
        {fetching ? (
          <div className="border border-hairline p-12 text-center text-bone/60 font-light">Chargement…</div>
        ) : filtered.length === 0 ? (
          <div className="border border-hairline p-12 text-center text-bone/60 font-light">
            Aucune alerte {filter === "pending" ? "en attente" : filter === "notified" ? "notifiée" : ""}.
          </div>
        ) : (
          <div className="border border-hairline divide-y divide-hairline">
            ...
            {filtered.map((a) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="font-serif text-bone">{a.product_name ?? a.product_slug ?? "—"}</div>
                    {a.product_slug && (
                      <div className="text-bone/40 text-[10px] tracking-wider">{a.product_slug}</div>
                    )}
                  </div>
                  <div className="text-bone/80 text-sm font-light break-all">
                    {a.email && <div>{a.email}</div>}
                    {a.whatsapp && <div className="text-bone/60">WhatsApp: {a.whatsapp}</div>}
                    {!a.email && !a.whatsapp && <span className="text-bone/40">—</span>}
                  </div>
                  <div className="eyebrow text-bone/70 text-[10px]">{a.size ?? "—"}</div>
                  <div className="text-bone/60 text-xs font-light">{dateStr}</div>
                  <div>
                    <span
                  ...
                  <div className="flex md:justify-end gap-3 flex-wrap">
                    ...
                    {!a.notified && (a.product_id || a.product_slug) && (
                      <button
                        type="button"
                        onClick={() => markAllForProduct(a.product_id, a.product_slug)}
